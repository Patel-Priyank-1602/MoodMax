...
import { analyzeBatch, downloadBatchReport } from '../api/client';
import {
...
export default function Batch() {
  ...
  const handleDownloadPdfReport = async () => {
    if (!summary || !results || results.length === 0) return;
    setIsGeneratingPdf(true);
    try {
      const blobData = await downloadBatchReport({
        filename: filename || 'batch_analysis.csv',
        summary,
        results,
      });
      ...
      const link = document.createElement('a');
      const baseName = filename ? filename.replace(/\.[^/.]+$/, "") : "batch";
      const downloadName = `MoodMax_Executive_Report_${baseName}.pdf`;
      link.setAttribute('href', url);
      link.setAttribute('download', downloadName);
      document.body.appendChild(link);
      link.click();
      ...
      setError('Failed to generate PDF report. Please check the backend connection.');
    } finally {
          ...
          <div className="batch-results-header">
            ...
            <div className="batch-actions-group">
              <button
                ...
                title="Generate 2-Page Executive PDF Audit Report"
              >
                {isGeneratingPdf ? (
                  <>
